...
format_host_ideogram_metadata <- function(sample) {
    ...
    paste0("**Strain/taxon name:** ", host_name, "\n\n",
...
format_defense_finder_output <- function(defense_finder_data) {
    required_cols <- c("sys_id", "type", "subtype", "sys_beg", "sys_end",
...
process_proviruses <- function(data_genomad) {
    ...
    proviruses <- data_genomad %>%
        ...
        dplyr::mutate(contig = sub("\\|provirus_.*", "", seq_name)) %>%
...
plot_genome_ideogram <- function(genome_current, proviruses_gr_features, host_display_label = genome_current) {
    ...
    # Keep only provirus features located on contigs that exist in the host FASTA.
    feature_contigs <- as.character(GenomeInfoDb::seqnames(proviruses_gr_features))
    keep_features <- feature_contigs %in% original_seqlevels
    ...
    # Rename genome and feature seqlevels with the same original FASTA-derived map.
    ...
    tryCatch(
        {
            ...
            if (length(GenomeInfoDb::seqlevels(host_genome_ideogram)) == 1) {
                ...
                virus_containing_contigs <- paste0("c", as.numeric(factor(virus_containing_contigs)))
