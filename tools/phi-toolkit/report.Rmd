...
extract_host_name_from_gtdbtk <- function(sample) {
    host_name <- sample

    if (exists("data_gtdbtk_host", inherits = TRUE)) {
        host_taxonomy <- data_gtdbtk_host %>%
            dplyr::filter(user_genome == sample) %>%
            dplyr::pull(classification)

        if (length(host_taxonomy) > 0 && !is.na(host_taxonomy[1]) && host_taxonomy[1] != "") {
            last_taxon <- extract_last_known_taxonomy(host_taxonomy[1])
            host_name <- stringr::str_replace_all(last_taxon$name, "_", " ")
            if (is.na(host_name) || host_name == "") host_name <- sample
        }
    }
...
# Infer a genome accession from the submitted genome name or
# from the corresponding FASTA header. If no genome-level accession can be obtained return "NA".
infer_host_accession <- function(sample) {
    accession_regex <- "^(NC|NZ|AC|CP|CM|CU|AE|LT|LN|LR|AP|BA|GCA|GCF)_[A-Za-z0-9]+(\\.[0-9]+)?$"

    # If the sample name itself is already an accession, use it.
    if (!is.na(sample) && stringr::str_detect(sample, accession_regex)) {
        return(sample)
    }

    fasta_file_path <- file.path(params$outdir, "genomes", paste0(sample, ".fna"))
    if (!file.exists(fasta_file_path)) {
        return("NA")
    }

    fasta_headers <- readLines(fasta_file_path, warn = FALSE)
    fasta_headers <- fasta_headers[stringr::str_starts(fasta_headers, ">")]
    if (length(fasta_headers) == 0) {
        return("NA")
    }

    # Use the first FASTA token, stripping ">" and any description text.
    fasta_ids <- fasta_headers %>%
        stringr::str_remove("^>") %>%
        stringr::str_replace("\\s.*$", "")

    if (length(fasta_ids) == 1 && stringr::str_detect(fasta_ids[1], accession_regex)) {
        return(fasta_ids[1])
    }

    # For complete genomes, geNomad provirus records may use names such as
    # NC_003228.3|provirus_2674949_2724741. Use this only when a single unique
    # accession-like contig is found among coordinate-defined provirus records.
    genomad_summary_path <- file.path(
        params$outdir, "host_analyses", "genomad", sample,
        paste0(sample, "_summary"), paste0(sample, "_virus_summary.tsv")
    )

    if (file.exists(genomad_summary_path)) {
        genomad_summary <- tryCatch(
            readr::read_tsv(genomad_summary_path, show_col_types = FALSE) %>% janitor::clean_names(),
            error = function(e) NULL
        )

        if (!is.null(genomad_summary) && all(c("seq_name", "coordinates") %in% colnames(genomad_summary))) {
            candidate_accessions <- genomad_summary %>%
                dplyr::filter(!is.na(coordinates), coordinates != "", coordinates != "NA", coordinates != ".") %>%
                dplyr::mutate(contig_id = stringr::str_replace(seq_name, "\\|provirus_.*$", "")) %>%
                dplyr::pull(contig_id) %>%
                unique()

            candidate_accessions <- candidate_accessions[stringr::str_detect(candidate_accessions, accession_regex)]
            if (length(candidate_accessions) == 1) {
                return(candidate_accessions[1])
            }
        }
    }
...
format_host_display_label <- function(sample) {
    host_name <- extract_host_name_from_gtdbtk(sample)
    accession <- infer_host_accession(sample)
    label_id <- ifelse(!is.na(accession) && accession != "NA" && accession != "", accession, sample)
    paste0(host_name, " (accession/sample ID: ", label_id, ")")
}

# Return a visible two-line metadata block for the host ideogram.
format_host_ideogram_metadata <- function(sample) {
    host_name <- extract_host_name_from_gtdbtk(sample)
    accession <- infer_host_accession(sample)

    paste0(
        "**Strain/taxon name:** ", host_name, "\n\n",
...
```{r report_specific_helpers, echo=FALSE}
# DefenseFinder output headers renamed
format_defense_finder_output <- function(defense_finder_data) {
    required_cols <- c(
        "sys_id", "type", "subtype", "sys_beg", "sys_end",
        "protein_in_syst", "genes_count", "name_of_profiles_in_sys"
    )

    missing_cols <- setdiff(required_cols, names(defense_finder_data))
    if (length(missing_cols) > 0) {
        log_debug(paste("DefenseFinder table missing columns:", paste(missing_cols, collapse = ", ")))
        return(defense_finder_data)
    }
    ...
    defense_finder_data %>%
        ...
        dplyr::rename(
            ...
            `Type` = type,
            `Subtype` = subtype,
            `System_start` = sys_beg,
            `System_end` = sys_end,
            `Proteins in system` = protein_in_syst,
            `Gene count` = genes_count,
            `Profile names in system` = name_of_profiles_in_sys
        )
}
```
...
# Handling of missing/NA geNomad coordinates and fragmented MAGs
# Normalize sequence/contig names for matching geNomad output to FASTA-derived ideogram names.
# This avoids false mismatches when FASTA headers include descriptions, e.g.
# ">NC_000913.3 Escherichia coli ...", while geNomad reports
# "NC_000913.3|provirus_...".
normalize_contig_id <- function(x) {
    x <- as.character(x)
    x <- stringr::str_replace(x, "\\|provirus_.*$", "")
    x <- stringr::str_replace(x, "\\s.*$", "")
    x
}

# Process proviruses data
# Keep coordinate-defined provirus regions, but keep the ORIGINAL normalized
# contig name. The conversion to c1/c2/... is done later using the FASTA order.
process_proviruses <- function(data_genomad) {
    if (is.null(data_genomad) || nrow(data_genomad) == 0) {
        log_debug("No geNomad data available for process_proviruses")
        return(GRanges())
    }
    ...
    proviruses <- data_genomad %>%
        dplyr::filter(
            ...
            coordinates != "NA",
            coordinates != ".",
            stringr::str_detect(coordinates, "^[0-9]+-[0-9]+$")
        ) %>%
        dplyr::mutate(contig = normalize_contig_id(seq_name)) %>%
        dplyr::select(seq_name, coordinates, length, contig, virus_score, n_hallmarks) %>%
        tidyr::separate(coordinates, into = c("start", "end"), sep = "-", remove = FALSE) %>%
        dplyr::mutate(
    ...
    if (nrow(proviruses) == 0) {
        log_debug("No coordinate-defined provirus regions available for ideogram")
        return(GRanges())
    }
    ...
    proviruses_gr_features$length <- as.numeric(proviruses$end - proviruses$start + 1)
    ...
    proviruses_gr_features$seq_name <- proviruses$seq_name
...
plot_genome_ideogram <- function(genome_current, proviruses_gr_features, host_display_label = genome_current) {
    if (length(proviruses_gr_features) == 0) {
        cat("No coordinate-defined provirus regions were available for this sample. Candidate viral sequences are shown in the Genomad and CheckV summary table, but they cannot be placed on the host-genome ideogram because genomic coordinates were not defined. This situation can occur for fragmented assemblies or MAG-derived viral-like contigs.\n\n")
    ...
    original_seqlevels <- as.character(GenomeInfoDb::seqlevels(genome_ideogram))
    original_tokens <- normalize_contig_id(original_seqlevels)

    feature_tokens <- normalize_contig_id(as.character(GenomeInfoDb::seqnames(proviruses_gr_features)))
    matched_idx <- match(feature_tokens, original_tokens)
    keep_features <- !is.na(matched_idx)

    # If a genome has only one FASTA sequence, allow coordinate-defined proviruses
    # to be placed on that sequence even when the FASTA header and geNomad contig
    # name differ by description text or minor formatting.
    if (!any(keep_features) && length(original_seqlevels) == 1) {
        matched_idx <- rep(1L, length(proviruses_gr_features))
        keep_features <- rep(TRUE, length(proviruses_gr_features))
    }

    if (!any(keep_features)) {
        cat("Coordinate-defined provirus regions were reported, but their contig names could not be matched to the host-genome FASTA. Candidate viral sequences are shown in the summary table, but the host-genome ideogram was skipped to avoid plotting inconsistent coordinates.\n\n")
        return(invisible(NULL))
    }

    proviruses_gr_features <- proviruses_gr_features[keep_features]
    matched_seqlevels <- original_seqlevels[matched_idx[keep_features]]

    # Rebuild the GRanges with the FASTA-derived seqlevel names. This keeps the
    # plotting coordinates but ensures that feature seqnames and ideogram seqlevels
    # refer to the same sequences before both are renamed to c1/c2/...
    proviruses_gr_features <- GRanges(
        seqnames = matched_seqlevels,
        ranges = IRanges(
            start = as.numeric(start(proviruses_gr_features)),
            end = as.numeric(end(proviruses_gr_features))
        ),
        length = as.numeric(proviruses_gr_features$length),
        score = as.numeric(proviruses_gr_features$score),
        n_hallmarks = as.numeric(proviruses_gr_features$n_hallmarks),
        seq_name = proviruses_gr_features$seq_name
    )

    # Remove impossible coordinates when sequence lengths are available.
    seq_lengths <- GenomeInfoDb::seqlengths(genome_ideogram)
    if (!is.null(seq_lengths) && any(!is.na(seq_lengths))) {
        feature_seq <- as.character(GenomeInfoDb::seqnames(proviruses_gr_features))
        feature_bounds <- seq_lengths[feature_seq]
        in_bounds <- is.na(feature_bounds) |
            (as.numeric(start(proviruses_gr_features)) >= 1 &
                as.numeric(end(proviruses_gr_features)) <= as.numeric(feature_bounds))
        proviruses_gr_features <- proviruses_gr_features[in_bounds]
    }

    if (length(proviruses_gr_features) == 0) {
        cat("Coordinate-defined provirus regions were reported, but the coordinates were outside the host FASTA sequence lengths. Candidate viral sequences are shown in the summary table, but the host-genome ideogram was skipped.\n\n")
        return(invisible(NULL))
    }

    new_seqlevels <- paste0("c", seq_along(original_seqlevels))
    names(new_seqlevels) <- original_seqlevels

    genome_ideogram <- GenomeInfoDb::renameSeqlevels(genome_ideogram, new_seqlevels)
    feature_rename_map <- new_seqlevels[names(new_seqlevels) %in% GenomeInfoDb::seqlevels(proviruses_gr_features)]
    proviruses_gr_features <- GenomeInfoDb::renameSeqlevels(proviruses_gr_features, feature_rename_map)
    proviruses_gr_features <- GenomeInfoDb::keepSeqlevels(
        proviruses_gr_features,
        intersect(GenomeInfoDb::seqlevels(proviruses_gr_features), GenomeInfoDb::seqlevels(genome_ideogram)),
        pruning.mode = "coarse"
    )

    if (length(proviruses_gr_features) == 0) {
        cat("No coordinate-defined provirus regions could be matched to the renamed host-genome ideogram. Candidate viral sequences are shown in the summary table, but the host-genome ideogram was skipped.\n\n")
        return(invisible(NULL))
    }
    ...
    valid_sectors <- circlize::get.all.sector.index()
    proviruses_gr_features <- proviruses_gr_features[as.character(GenomeInfoDb::seqnames(proviruses_gr_features)) %in% valid_sectors]
    if (length(proviruses_gr_features) == 0) {
        cat("No coordinate-defined provirus regions could be matched to initialized ideogram sectors. Candidate viral sequences are shown in the summary table, but the host-genome ideogram was skipped.\n\n")
        return(invisible(NULL))
    }

    for (i in seq_along(proviruses_gr_features)) {
        name <- as.character(GenomeInfoDb::seqnames(proviruses_gr_features[i]))
        feature_start <- as.numeric(start(proviruses_gr_features[i]))
        feature_end <- as.numeric(end(proviruses_gr_features[i]))
        region <- data.frame(start = feature_start, end = feature_end)
    ...
    # changes to unit labels usch as Kb/Mb format
    feature_lengths <- as.numeric(proviruses_gr_features$length)
    feature_lengths <- ifelse(feature_lengths > 1000000,
        paste0(round(feature_lengths / 1000000, 2), " Mb"),
        paste0(round(feature_lengths / 1000, 2), " Kb")
    )
    labels <- paste0(as.character(GenomeInfoDb::seqnames(proviruses_gr_features)), " (", feature_lengths, ")")
    circlize::circos.labels(
        sectors = as.character(GenomeInfoDb::seqnames(proviruses_gr_features)),
        x = as.numeric(start(proviruses_gr_features)),
        labels,
        facing = "clockwise"
    )
    #title(main = paste0("Candidate prophages in ", host_display_label), cex.main = 0.9)
}
    ...
    tryCatch(
        {
            ...
            host_display_label <- format_host_display_label(sample)
            ...
            # Display DefenseFinder output as a table with human-readable headers
            if (!is.null(defense_finder_data) && nrow(defense_finder_data) > 0) {
                cat("**DefenseFinder Systems**:\n\n")

                defense_finder_tbl <- format_defense_finder_output(defense_finder_data)
                ...
                cat("No DefenseFinder systems detected.\n\n")
            }

            if (is.null(genomad_summary) || nrow(genomad_summary) == 0) {
                log_debug(paste("No geNomad summary data found for sample:", sample))
                cat("No candidate viral or prophage sequences were reported by geNomad for this sample. Downstream viral analyses are therefore unavailable for this sample.\n\n")
                return()
            }
            ...
            if (length(GenomeInfoDb::seqlevels(host_genome_ideogram)) == 1) {
                ...
                virus_containing_contigs <- unique(normalize_contig_id(genomad_summary$seq_name))
                host_seqlevels <- as.character(GenomeInfoDb::seqlevels(host_genome_ideogram))
                host_tokens <- normalize_contig_id(host_seqlevels)
                matched_host_contigs <- host_seqlevels[match(virus_containing_contigs, host_tokens)]
                matched_host_contigs <- matched_host_contigs[!is.na(matched_host_contigs)]

                if (length(matched_host_contigs) > 0) {
                    filtered_host_genome <- subset_and_update_ideogram(host_genome_ideogram, matched_host_contigs)
            ...
            checkv_for_join <- if (!is.null(checkv_data) && nrow(checkv_data) > 0) {
                checkv_data %>% dplyr::select(contig_id, gene_count, viral_genes, checkv_quality, miuvig_quality)
            } else {
                tibble::tibble(contig_id = character(), gene_count = numeric(), viral_genes = numeric(), checkv_quality = character(), miuvig_quality = character())
            }
            genomad_summary %>%
            ...
            cat(format_host_ideogram_metadata(sample))
            tryCatch(
                {
                    plot_genome_ideogram(sample, proviruses_gr_features, host_display_label = host_display_label)
                },
                error = function(e) {
                    log_debug(paste("Ideogram skipped for", sample, ":", conditionMessage(e)))
                    cat("Host-genome ideogram could not be rendered for this sample because coordinate-defined prophage intervals could not be safely matched to the host genome. Candidate viral sequences remain available in the summary tables.\n\n")
                }
            )
...
If multiple contigs are present in the genome, each will be shown with a distinct label (e.g., **c1**, **c2**, etc.). \n\n')
            ...
            if (!is.null(abricate_data) && nrow(abricate_data) > 0) {
            ...
            if (!is.null(iphop_data) && nrow(iphop_data) > 0) {
            ...
            if (!is.null(vibrant_data) && nrow(vibrant_data) > 0) {
        ...
        error = function(e) {
            ...
            cat(paste("Sample", sample, "could not be fully rendered. Some tables may still be available above; details were written to the debug log.\n\n"))
        }
    )
}
...
process_phage <- function(virus, genomad_summary, genomad_annotation, checkv_data, host_genome_size) {
    ...
    # phage-level ideogram drawn only for coordinate-defined provirus boundaries
    has_provirus_coordinates <- !is.na(virus$coordinates) &&
        virus$coordinates != "NA" &&
        stringr::str_detect(virus$coordinates, "^[0-9]+-[0-9]+$") &&
        stringr::str_detect(virus$seq_name, "\\|provirus_[0-9]+_[0-9]+")

    if (has_provirus_coordinates) {
        ...
        provirus_start <- NA_real_
        provirus_end <- NA_real_
        virus_length <- as.numeric(virus$length)
        current_contig_base <- current_contig
        current_provirus_range <- NA_character_
    }

    if (!is.null(genomad_annotation) && nrow(genomad_annotation) > 0 && "gene" %in% names(genomad_annotation)) {
        if (has_provirus_coordinates) {
            ...
            current_annotations <- genomad_annotation[grepl(paste0("^", current_contig_base), genomad_annotation$gene), ]
        }
        if (nrow(current_annotations) > 0 && "strand" %in% names(current_annotations)) {
            current_annotations <- current_annotations %>%
                dplyr::mutate(arrow_pos = ifelse(strand == -1, "start", "end"))
        }
    } else {
        current_annotations <- tibble::tibble()
    }
    ...
    if (has_provirus_coordinates) {
        ...
        cat("This candidate does not have coordinate-defined provirus boundaries in the geNomad output. It is therefore reported in the summary tables, but no host-genome ideogram is drawn for this candidate. This situation can occur when a viral sequence is assembled as an independent viral-like contig rather than as a bounded provirus interval within a longer host contig.\n\n")
    }

    cat("\n\n")
    cat("\n\n**Genes Annotation (geNomad):**\n\n")

    required_gene_cols <- c("gene", "length", "marker", "annotation_accessions", "annotation_description")
    if (nrow(current_annotations) > 0 && all(required_gene_cols %in% names(current_annotations))) {
        current_annotations %>%
            dplyr::select(dplyr::all_of(required_gene_cols)) %>%
            kableExtra::kbl() %>%
            kableExtra::kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive")) %>%
        ...
        cat("No geNomad gene annotation table was available for this candidate.\n\n")
    }
}
    ...
    tryCatch(
        {
            ...
            # numeric values inside circular plots use spaced Kb/Mb labels
            ...
            # numerical values inside the ring use spaced Kb labels
...
subset_and_update_ideogram <- function(ideogram, contigs) {
    contigs <- unique(as.character(contigs))
    contigs <- contigs[!is.na(contigs) & contigs != ""]
    keep <- as.character(GenomeInfoDb::seqnames(ideogram)) %in% contigs
    filtered <- ideogram[keep]
    if (length(filtered) == 0) {
        return(filtered)
    }
    GenomeInfoDb::keepSeqlevels(filtered, unique(as.character(GenomeInfoDb::seqnames(filtered))), pruning.mode = "coarse")
}
